...
export default function ShareButton({ articleId, userId, articleUrl, articleTitle }: ShareButtonProps) {
	...
	useEffect(() => {
		const loadSharesCount = async () => {
			try {
				const count = await getSharesCount(articleId);
	...
	return (
		<div className="relative">
			...
			{showShareMenu && (
				<div className={`absolute right-0 mt-2 w-56 rounded-lg shadow-xl border py-2 z-10 ${
					isDark
					...
					>
						<svg className={`w-5 h-5 ${isDark ? 'text-slate-400' : 'text-gray-600'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
