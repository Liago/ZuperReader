-- ============================================
-- MIGRATION: User Reading Preferences (CLEAN VERSION)
-- ============================================
-- This migration drops and recreates the user_preferences table
-- Use this if the initial migration had issues

-- 1. Drop existing table if it exists (with CASCADE to remove dependencies)
DROP TABLE IF EXISTS user_preferences CASCADE;

-- 2. Drop the trigger function if it exists
DROP FUNCTION IF EXISTS update_user_preferences_updated_at() CASCADE;

-- 3. Create user_preferences table with correct structure
CREATE TABLE user_preferences (
    id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
    -- Reading preferences (using snake_case for PostgreSQL convention)
...
-- 4. Create index for faster user lookups
CREATE INDEX idx_user_preferences_id ON user_preferences(id);
